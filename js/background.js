...
function prepareTreeDraw(spriteInfo) {
  ...
  ctx.drawImage(
    ...
    spriteInfo.spriteX,
    spriteInfo.spriteY,
    spriteInfo.spriteWidth,
    spriteInfo.spriteHeight,
    0,
    0,
    canvas.width,
    canvas.height
  ...
  spriteInfo["draw"] = function (mainCtx, treeInfo) {
    mainCtx.drawImage(
      ...
      canvas.height,
      treeInfo.left,
      mainCtx.canvas.height - treeInfo.bottom - canvas.height,
      canvas.width,
      canvas.height
    );
  };
...
function updateForeTrees(difficulty) {
  ...
  if (
    Math.random() < 0.15 &&
    lastTree.left + lastTree.sprite.width < 750 + 200 * difficulty
  ) {
    foreTrees.push({
      left: 800,
      bottom: -50,
      front: Math.random() < 0.5,
      sprite: Math.random() < 0.5 ? firstForeTree : secondForeTree,
    });
  }
}
...
function updateGroundAndBackTrees(difficulty) {
  ...
  if (
    ...
    backTrees.push({
      ...
      bottom: 30,
      front: Math.random() < 0.5,
      sprite: Math.random() < 0.5 ? firstBackTree : secondBackTree,
    });
  }
}
...
function prepareBackground(canvasWidth, canvasHeight) {
  ...
  drawBackGround = prepareBackgroundDraw(
    back,
    canvasWidth,
    canvasHeight,
  ...
  drawMiddleGround = prepareBackgroundDraw(
    middle,
    canvasWidth,
    canvasHeight,
  ...
  drawFrontGround = prepareBackgroundDraw(
    front,
    canvasWidth,
    canvasHeight,
